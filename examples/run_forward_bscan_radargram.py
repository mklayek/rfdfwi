...
#
# GPR B-scan (radargram) — standalone script.
#
...
# =============================================================================
"""
GPR B-scan (radargram) — GERMAINE FD2TD style.

Usage:
    python examples/run_forward_bscan_radargram.py --stag2 --ncpus 10 --model-name clay_lens
...
DEFAULT_OUT = root / "results" / "bscan_radargram"
BSCAN_DPI = 600
...
def _parse_args():
    p = argparse.ArgumentParser(
        description="GPR B-scan radargram (GERMAINE FD2TD style).",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    add_common_args(p, default_kind="forward_bscan_radargram")
...
def main():
    ...
    if args.config:
        ...
        sg_config = root / "input" / "input_shotgather.yaml"
        config_path = sg_config if sg_config.exists() else root / "input" / "input_forward.yaml"
    ...
    model_name = args.model_name or model_cfg.get("type", "model")
    ...
    # ---- Build model ----
    if model_cfg.get("type") == "layered_layek2021":
