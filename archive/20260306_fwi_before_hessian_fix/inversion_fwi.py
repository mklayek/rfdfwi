...
def run_inversion(
    ...
    for it in range(max_iter):
        ...
        # ---- Hessian preconditioning + search direction ----
        # Divide by pseudo-Hessian diagonal to rescale gradient to O(1).
        # Without this, raw grad_epsr ~ 1e+20 makes auto-step ~ 1e-42 → no update.
        H_max = max(float(hess_epsr.max()), float(hess_sigma.max()), 1e-300)
        H_eps = 1e-5 * H_max
        dir_epsr  = -g_epsr  / (hess_epsr  + H_eps)
        dir_sigma = -g_sigma / (hess_sigma + H_eps)
        print(f"  Hessian precond: H_max={H_max:.3e}  H_eps={H_eps:.3e}"
              f"  |d_epsr|_max={np.max(np.abs(dir_epsr)):.3e}"
              f"  |d_sigma|_max={np.max(np.abs(dir_sigma)):.3e}")

        # ---- Initial step size ----
        # H-preconditioned direction is already O(1) — start at step = 1.0.
        if step <= 0.0 or it == 0:
            step = 1.0
            print(f"  step={step:.3e} [init=1.0, H-preconditioned]")

        # ---- Armijo backtracking line search ----
        # Correct descent slope: g·d  (negative because d = -g/H)
        descent_slope = float(np.sum(g_epsr * dir_epsr + g_sigma * dir_sigma))
        phi0       = L2
